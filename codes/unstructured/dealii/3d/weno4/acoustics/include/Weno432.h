...
class Weno4_3D
{
    ...
    LA::MPI::Vector local_U;
    LA::MPI::Vector local_V;

    // Coefficients for WENO polynomials
    std::vector<Vector<double>> coeffs_U;
    std::vector<Vector<double>> upwind_coeffs_U;

    LA::MPI::Vector error_U;
    LA::MPI::Vector local_error_U;
...
public:
    Weno4_3D(double, double, bool, unsigned int, double);
    void run();
};
