...
//    auto start = std::chrono::system_clock::now();
	...
	error_U.reinit(locally_owned_dofs, locally_relevant_dofs, MPI_COMM_WORLD);
	local_error_U.reinit(locally_owned_dofs, MPI_COMM_WORLD);

    coeffs_U.resize(n_relevant_cells);

	upwind_coeffs_U.resize(n_relevant_cells);
    ...
    for (unsigned int i = 0; i < n_relevant_cells; i++) {
        ...
        coeffs_U[i].reinit(20);
        upwind_coeffs_U[i].reinit(20);
