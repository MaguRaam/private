...
//                    pcout<<"Fx_v: "<<local_Fx_v<<"\tc: "<<c<<"\tU: "<<U(1)<<"\tgradU: "<<gradU[1]<<"\tgradV: "<<gradU[2]<<"\tgradW: "<<gradU[3]<<std::endl;
				}
			}
		}
...
/*
    std::cout<<"Fx_v: "<<local_Fx_v<<"\tnormalized: "<<local_Fx_v/(0.5*claw.gamma()*M*M)<<std::endl;
...
*/
	...
	double Fx_p = Utilities::MPI::sum (local_Fx_p, MPI_COMM_WORLD);
	double Fy_p = Utilities::MPI::sum (local_Fy_p, MPI_COMM_WORLD);
	double Fz_p = Utilities::MPI::sum (local_Fz_p, MPI_COMM_WORLD);

	double Fx_v = Utilities::MPI::sum (local_Fx_v, MPI_COMM_WORLD);
	double Fy_v = Utilities::MPI::sum (local_Fy_v, MPI_COMM_WORLD);
	double Fz_v = Utilities::MPI::sum (local_Fz_v, MPI_COMM_WORLD);
